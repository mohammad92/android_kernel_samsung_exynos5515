...
#if IS_ENABLED(CONFIG_SAMSUNG_NFC)
int p61_device_match_of_node(struct device *dev, void *np)
{
	return dev->of_node == np;
}

static inline struct device *
p61_bus_find_device_by_of_node(struct bus_type *bus, struct device_node *np)
{
	return bus_find_device(bus, NULL, np, p61_device_match_of_node);
}
#endif
...
#if IS_ENABLED(CONFIG_SAMSUNG_NFC)
		struct device *nfc_dev = p61_bus_find_device_by_of_node(&i2c_bus_type, p61_dev->nfc_node);
		if (!nfc_dev) {
			  ...
			  unsigned long arg)
{
	...
	P61_DBG_MSG(KERN_ALERT "Enter %u arg = %ld\n", cmd, arg);
	p61_dev = filp->private_data;
	...
	case PERFORM_RESET_PROTECTION_COMPAT:
		...
		NFC_LOG_INFO("PERFORM_RESET_PROTECTION ret: %d exit\n", ret);
		break;
	case ESE_SET_TRUSTED_ACCESS_COMPAT:
		NFC_LOG_INFO("Enter %s: TRUSTED access enabled=%d\n", __func__, arg);
		if (arg == 1) {
			NFC_LOG_INFO("ESE_SET_TRUSTED_ACCESS: enter Enabling\n");
			gpio_set_value(p61_dev->trusted_ese_gpio, 1);
	...
	P61_DBG_MSG(KERN_ALERT "exit %u arg = %lu\n", cmd, arg);
	return ret;
}
#endif
