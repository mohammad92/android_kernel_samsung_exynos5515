...
static int hideep_setup_bezel_input(struct hideep_t *ts)
{
	...
	input_set_abs_params(ts->wheel_dev, ABS_MISC, 0, 47, 0, 0);
...
mode_change_failed:
	input_info(true, &ts->client->dev, "%s: fail %d\n", __func__, ret);
...
static int hideep_parse_dts(struct device *dev, struct hideep_platform_data_t *pdata)
{
	...
	of_property_read_string(np, "hideep,firmware_name", &pdata->firmware_name);
	...
	input_info(true, dev, "%s: max(%d/%d) FW:%s, HW reset:%d, ldo control:%d Bezel:%d Bringup:%d\n",
			__func__, pdata->max_x, pdata->max_y, pdata->firmware_name,
			pdata->support_reset_control,	pdata->support_ldo_control,
			pdata->support_bezel_detent, pdata->bringup);
