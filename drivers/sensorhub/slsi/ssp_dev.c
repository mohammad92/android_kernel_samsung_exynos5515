	...
	struct ssp_data *data)
{
	...
	data->reg_hrm5v= devm_regulator_get(dev, "vdd_hrm");
	if (IS_ERR(data->reg_hrm5v)) {
		pr_err("[SSP] could not get bst1_reg, %ld\n", PTR_ERR(data->reg_hrm5v));
		data->reg_hrm5v = 0;
	} else {
		errorno = regulator_enable(data->reg_hrm5v);
		if (errorno) {
			dev_err(dev, "Failed to enable reg_hrm5v regulators: %d\n",	errorno);
		}
	}
#endif
...
int check_hrm_vendor(struct ssp_data *data, struct device *dev)
{
	...
	ssp_regulator_ctrl(data->reg_hrm5v, true, 0);
	ssp_regulator_ctrl(data->reg_hrm3p3v, true, 100);
	...
	if (data->hrm_vender) {
		...
		ssp_regulator_ctrl(data->reg_hrm1p8v, true, 0);
		ssp_regulator_ctrl(data->reg_hrm5v, true, 100);
		data->hrm_vender = 0;
	}
	return 0;
#endif
...
int get_sensor_state(struct ssp_data *data, u16 sensor_type, const char *func_name)
{
	...
	if (!sensor_state)
		pr_info("[SSP]: %s - temperature sensor is not connected(0x%llx)\n",
			func_name, data->uSensorState);
