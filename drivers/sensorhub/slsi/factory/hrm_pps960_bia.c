...
static int read_bia_nv_param(struct ssp_data *data)
{
	int iRet = 0;
	struct file *cal_filp = NULL;
	mm_segment_t old_fs;
	u32 nv_param[BIA_CAL_DATA_SIZE];
	...
	iRet = vfs_read(cal_filp, (char *)&nv_param, BIA_CAL_DATA_SIZE * sizeof(u32), &cal_filp->f_pos);
	...
	pr_info("[SSP] %s: [%x %x %x %x %x %x %x %x]\n", __func__,
		nv_param[0], nv_param[1], nv_param[2], nv_param[3],
		nv_param[4], nv_param[5], nv_param[6], nv_param[7]);
	pr_info("[SSP] %s: [%x %x %x %x %x %x %x %x]\n", __func__,
		 nv_param[8], nv_param[9], nv_param[10], nv_param[11],
		 nv_param[12], nv_param[13], nv_param[14], nv_param[15]);
	pr_info("[SSP] %s: [%x %x %x %x %x %x %x %x]\n", __func__,
		 nv_param[16], nv_param[17], nv_param[18], nv_param[19],
		 nv_param[20], nv_param[21], nv_param[22], nv_param[23]);
	pr_info("[SSP] %s: [%x %x %x %x %x %x %x %x]\n", __func__,
		 nv_param[24], nv_param[25], nv_param[26], nv_param[27],
		 nv_param[28], nv_param[29], nv_param[30], nv_param[31]);

	memcpy(data->biacal, nv_param, sizeof(nv_param));
