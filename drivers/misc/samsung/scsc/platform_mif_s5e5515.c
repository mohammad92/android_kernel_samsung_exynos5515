...
static bool disable_apm_setup = false;
...
//	(void)platform;
	...
	ret = s2mpw03_write_reg(client, 0x4B, 0xDC);
	pr_info("write_reg ret : %d\n", ret);
	ret = s2mpw03_read_reg(client, 0x4B, &val);
	pr_info("read_reg ret : %d, val : 0x%x\n", ret, val);

	/*
	 * /sys/kernel/debug/s2mpw03-regs # echo 0x01 0x4B 0xDC > i2cadata
	 * /sys/kernel/debug/s2mpw03-regs # cat i2cdata
	 */
}
