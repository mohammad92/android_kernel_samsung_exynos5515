...
void slsi_rx_event_log_indication(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	char *string = NULL;
	bool multi_param = false, current_info = true, valid_data = false;
	u32 auth_algo_type = 0, sae_type = 0, sn = 0;
	int freq_list[MAX_FREQUENCY_COUNT] = {0};
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	int dialog_token = 0, disassoc_timer = 0, validity_time = 0, btm_cand_preference = 0;
	...
	case FAPI_EVENT_WIFI_EVENT_AUTH_RECEIVED:
		...
		slsi_conn_log2us_auth_resp(sdev, dev, mac_addr, auth_algo_type, sae_type, sn, status_code);
		break;
