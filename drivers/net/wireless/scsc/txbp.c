...
#define LOG_FILTER_RESOURCE            (LOG_FILTER_Q_STATUS << 1)
#define LOG_FILTER_AC_PRESENCE         (LOG_FILTER_RESOURCE << 1)
#define LOG_FILTER_TX_FAILURE          (LOG_FILTER_AC_PRESENCE << 1)
#define LOG_FILTER_TX_SCHEDULE         (LOG_FILTER_TX_FAILURE << 1)
...
struct txbp_data {
	rwlock_t cod_lock;
	int cod;
	atomic_t active_interfaces;
	struct list_head active_interface_list;
	u32 peers;
};
...
void slsi_tx_timeout(struct net_device *dev)
{
	...
	if (list_empty(&txbp_priv.active_interface_list))
		return;
...
static __always_inline int slsi_txbp_has_resource(struct tx_netdev_data *tx_priv)
{
	...
	/* Netdev domain */
	mod /= atomic_read(&txbp_priv.active_interfaces);
...
{
	u8 ac_presence = slsi_txbp_check_ac_presence(tx_priv);
	u32 ndev_mod = bp_mod / atomic_read(&txbp_priv.active_interfaces);
	...
	for (ac = SLSI_TRAFFIC_Q_VO; ac > -1; ac--) {
		...
		while (ac_budget_limit[ac]) {
			...
			if (ret != NETDEV_TX_OK) {
				...
				SLSI_DBG1(sdev, SLSI_TX, "Tx failed with ret:%d\n", ret);
				show_cod(tx_priv);
				if (bp_log & LOG_FILTER_TX_FAILURE)
					SLSI_NET_INFO(tx->ndev, "Tx failed remaining budget %u\n", ac_budget_limit[ac]);
...
void slsi_dev_attach_post(struct slsi_dev *sdev)
{
	...
	atomic_set(&txbp_priv.active_interfaces, 0);
	INIT_LIST_HEAD(&txbp_priv.active_interface_list);
	txbp_priv.peers = 0;
}
...
__always_inline bool slsi_vif_activated_post(struct slsi_dev *sdev, struct net_device *dev, struct netdev_vif *ndev_vif)
{
	...
	atomic_inc(&txbp_priv.active_interfaces);
	list_add(&tx_priv->list, &txbp_priv.active_interface_list);
	SLSI_NET_INFO(dev, "BP: active interfaces %u\n", atomic_read(&txbp_priv.active_interfaces));
					       ...
					       struct netdev_vif *ndev_vif)
{
	...
	atomic_dec(&txbp_priv.active_interfaces);
...
static __always_inline void slsi_txbp_schedule_napi(struct net_device *dev, struct tx_netdev_data *tx_priv)
{
	...
	if (bp_log & LOG_FILTER_TX_SCHEDULE)
		...
		if (ndev_vif->peer_sta_record[i] && ndev_vif->peer_sta_record[i]->valid) {
			...
			if (peer->qos_enabled)
				fapi_set_u16(duplicate_skb, u.ma_unitdata_req.flow_id,
					     ...
					     ((ndev_vif->peer_sta_record[i]->aid << 8) & 0xff00));
			else
				fapi_set_u16(duplicate_skb, u.ma_unitdata_req.flow_id,
...
int slsi_tx_done(struct slsi_dev *sdev, u32 colour, bool more)
{
	...
	if (!colour && !more) {
		write_lock(&txbp_priv.cod_lock);
		list_for_each_entry(tx_priv, &txbp_priv.active_interface_list, list) {
			for (ac = 0 ; ac < AC_CATEGORIES ; ac++) {
