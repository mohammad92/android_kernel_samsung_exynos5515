...
__always_inline struct hip_priv *slsi_lbm_get_hip_priv_from_napi(struct napi_struct *napi)
{
...
__always_inline struct hip_priv *slsi_lbm_get_hip_priv_from_work(struct work_struct *data)
{
					 ...
					 int irq, int idx)
{
	...
	if (!bh_tmp) {
		SLSI_ERR_NODEV("Cannot allocate bh\n");
		return NULL;
	}
