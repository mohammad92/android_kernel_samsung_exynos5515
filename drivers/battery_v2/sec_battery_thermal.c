...
static bool sec_bat_check_limit(bool now_state, int temp, int high_temp, int recv_temp)
{
	return (!now_state && (temp >= high_temp)) ||
		(now_state && (temp > recv_temp));
}
#endif
...
void sec_bat_check_bat_temp_limit(struct sec_battery_info *battery)
{
	...
	if (sec_bat_check_limit(battery->chg_limit, battery->temperature,
			btl->high_temp, btl->recv_temp)) {
		sec_vote(battery->fcc_vote, VOTER_HEAT_CONTROL,
			true, btl->fcc);
