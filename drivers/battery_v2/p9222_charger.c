...
static bool p9222_compatible_tx_check(struct p9222_charger_data *charger, int tx_id)
{
	...
	if (ptype != NULL) {
		charger->support_power_hold = ptype->support_power_hold;
		return true;
	}
...
static int p9222_get_target_vout(struct p9222_charger_data *charger)
{
	...
	if (p9222_get_pad_type(charger, charger->tx_id) != P9222_PAD_TYPE_MULTI)
		return target_vout;
...
static int p9222_normal_full_charge(struct p9222_charger_data *charger)
{
	...
	sec_vote(charger->apm_vote, VOTER_P9222_CONTROL, true, WPC_APM_VOUT_OFF);
	msleep(200);
	sec_vote(charger->ldo_vote, VOTER_P9222_CONTROL, true, WPC_LDO_OFF);
...
static int p9222_normal_re_charge(struct p9222_charger_data *charger)
{
	...
	sec_vote(charger->ldo_vote, VOTER_P9222_CONTROL, false, WPC_LDO_ON);
	sec_vote(charger->apm_vote, VOTER_P9222_CONTROL, true, WPC_APM_VOUT_ON);
	...
	sec_vote(charger->apm_vote, VOTER_P9222_CONTROL, false, WPC_APM_NONE);
...
static int p9222_set_phm_vote(void *data, int v)
{
	...
	case WPC_PHM_ON:
		...
		if (charger->support_power_hold == 1) {
			__pm_stay_awake(charger->power_hold_chk_ws);
			queue_delayed_work(charger->wqueue,
...
static int p9222_set_apm_vote(void *data, int v)
{
	...
	default:
	{
		...
		} else if (capacity >= charger->pdata->cc_cv_threshold) {
...
static int p9222_monitor_work(struct p9222_charger_data *charger)
{
	...
	if (!p9222_is_phm(charger)) {
		if (p9222_check_darkzone_state(charger, vrect, vout)) {
			...
			sec_vote(charger->phm_vote, VOTER_P9222_CONTROL, true, WPC_PHM_ON);
			sec_votef(VN_CHG_EN, VOTER_P9222_CONTROL, true, SEC_BAT_CHG_MODE_CHARGING_OFF);
			sec_votef(VN_CHG_EN, VOTER_P9222_CONTROL, false, false);
			sec_vote(charger->phm_vote, VOTER_P9222_CONTROL, false, WPC_PHM_OFF);
...
static void p9222_wpc_isr_work(struct work_struct *work)
{
	...
	case P9222_TX_DATA_COM_TX_ID:
		if (!is_wireless_type(charger->cable_type)) {
			...
			if (p9222_get_pad_type(charger, val_data) != P9222_PAD_TYPE_MULTI)
				queue_delayed_work(charger->wqueue, &charger->wpc_det_work, 0);
			break;
		...
		if (p9222_compatible_tx_check(charger, val_data)) {
			...
			pr_info("%s: data = 0x%x, type = %d, compatible TX pad\n",
				__func__, val_data, pad_type);
...
static void p9222_charger_shutdown(struct i2c_client *client)
{
	...
	ping_duration = (charger->power_hold_mode) ?
		PHM_DETACH_DELAY : p9222_get_ping_duration(charger);
	gpio_direction_output(charger->pdata->wpc_en, 1);
	msleep(ping_duration);
}
