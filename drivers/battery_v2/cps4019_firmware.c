...
static int run_fw(struct sb_wrl_fw *fw, struct sb_wrl_fw_data *app_bl, struct sb_wrl_fw_data *app)
{
	int ret;
...

{
	...
	fw_log("Step3, load firmware to MTP\n");
	p = (int *)app->data;
	...
	fw_log("Step5, write mcu start flag\n");
...
static int run_fw_by_sdcard(struct sb_wrl_fw *fw, const char *app_bl_path, const char *app_path)
{
	struct sb_wrl_fw_data app_bl, app;
	int ret, state = SB_WRL_FW_RESULT_FAIL;
	...
	if (ret < 0) {
		fw_log("failed to read firmware (%s, ret = %d)", app_bl_path, ret);
		goto get_fail;
	}

	ret = get_fw_data(app_path, &app);
	if (ret < 0) {
		fw_log("failed to read firmware (%s, ret = %d)", app_path, ret);
		goto get_fail;
	}

	state = run_fw(fw, &app_bl, &app);
...
static void cb_work(struct work_struct *work)
{
	...
	{
		struct sb_wrl_fw_data app2_bl, app2;

		app2_bl.data = CPS4019_APP2_BL;
		app2_bl.size = CPS4019_APP2_BL_SIZE;

		app2.data = CPS4019_APP2;
		app2.size = CPS4019_APP2_SIZE;

		state = run_fw(fw, &app2_bl, &app2);
	}
		break;
	case SB_WRL_RX_SDCARD_MODE:
	{
		state = run_fw_by_sdcard(fw, CPS4019_APP1_BL_PATH, CPS4019_APP1_PATH);
		state = run_fw_by_sdcard(fw, CPS4019_APP2_BL_PATH, CPS4019_APP2_PATH);
	}
		break;
	default:
