...
static int s2mpw03_set_in2bat(struct s2mpw03_charger_data *charger, int state)
{
	int ret = 0;

	/* Enable : 0, Disable : 1 */
	ret = s2mpw03_update_reg(charger->client,
		S2MPW03_CHG_REG_CHG_SET4,
		((state) ? (0) : (CHG_IN2BAT_MASK)), CHG_IN2BAT_MASK);

	pr_info("%s: set in2bat (%d)\n", __func__, state);
	return ret;
}
...
/*	int previous_cable_type = charger->cable_type; */
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (is_nocharge_type(charger->cable_type)) {
			...
			s2mpw03_set_in2bat(charger, true);
		} else {
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_FACTORY_MODE:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_CHG_IN2BAT:
			s2mpw03_set_in2bat(charger, val->intval);
			break;
		default:
			break;
		}
