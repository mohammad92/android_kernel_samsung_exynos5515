...
struct sec_bat_eng_data *sec_bat_init_eng_data(struct device *dev)
{
	...
	/* init */
	eng_data->temperature_test_battery = 0x7FFF;
...
void sec_bat_set_temp_test(struct sec_battery_info *battery)
{
	struct sec_bat_eng_data *eng_data = battery->eng_data;

	if (IS_ERR_OR_NULL(eng_data)) return;

	if (eng_data->temperature_test_battery > -300 && eng_data->temperature_test_battery < 3000) {
		pr_info("%s : battery temperature test %d\n", __func__, eng_data->temperature_test_battery);
		battery->temperature = eng_data->temperature_test_battery;
	}
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	switch (offset) {
	case BATT_TEMP_TEST:
		i += scnprintf(buf + i, PAGE_SIZE - i, "b:%d\n",
			eng_data->temperature_test_battery);
		break;
				...
				const char *buf, size_t count)
{
	...
	case BATT_TEMP_TEST:
	{
		...
		if (sscanf(buf, "%c %10d\n", &tc, &x) == 2) {
			...
			if (tc == 'b')
				eng_data->temperature_test_battery = x;
		}
	}
		break;
	case BATT_CHARGE_TEST:
