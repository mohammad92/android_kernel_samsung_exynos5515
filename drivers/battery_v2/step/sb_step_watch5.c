...
static int set_chg_in2bat(struct sb_step_watch5 *st, bool state)
{
	union power_supply_propval value = { state, };

	return psy_do_property(st->chg_name, set,
		POWER_SUPPLY_EXT_PROP_CHG_IN2BAT, value);
}

static void set_vote_by_vm(struct sb_step_watch5 *st, union sb_wrl_vm *vm, bool en)
{
	if ((vm == NULL) || (vm->base.type >= SB_WRL_VM_MAX)) {
		set_chg_in2bat(st, false);
		return;
	}

	/* set IN2BAT */
	set_chg_in2bat(st, (vm->base.type == SB_WRL_VM_DC));
...
{
/*
	struct sb_step_watch5 *st = container_of(nb, struct sb_step_watch5, sb_nb);
	sb_data *sbd = data;
...
static int cb_watch5_monitor(void *pdata, struct sb_step_data data)
{
	...
	case STEP_MODE_CC:
	case STEP_MODE_CC_ADJ:
		if (stc->fv - DEFAULT_VOLT_GAP > data.volt_avg) {
			st->mode = STEP_MODE_CC;
			goto step_end;
		}
