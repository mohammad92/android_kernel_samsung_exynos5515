		...
		bool on)
{
	...
	do {
		...
		val = readl(ioremap(0x13a50000, SZ_4K) + 0x14);
...
void abox_request_dram_on(struct device *dev, unsigned int id, bool on)
{
	...
	abox_info(dev, "abox 0x13a50010(0x%lu)\n", readl(ioremap(0x13a50000,
					SZ_4K) + 0x10));
	//ret = regmap_write(regmap, ABOX_SYSPOWER_CTRL, val);
	writel(val, ioremap(0x13a50000, SZ_4K) + 0x10);
...
static int abox_enable(struct device *dev)
{
	...
	abox_info(dev, "0x12863400(0x%lu)\n", readl(ioremap(0x12863000, SZ_4K) + 0x400));
	if (has_reset) {
		abox_cpu_power(true);
		writel(0x1, ioremap(0x12863000, SZ_4K) + 0x400);
		abox_cpu_enable(true);
	}
	abox_info(dev, "0x12863400(0x%lu)\n", readl(ioremap(0x12863000, SZ_4K)
				+ 0x400));
